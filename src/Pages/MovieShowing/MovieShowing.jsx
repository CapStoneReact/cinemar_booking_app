...
export default function MovieShowing() {
  ...
  useEffect(() => {
    dispatch(getMovieShowing(IDGroup));
  ...
  if (movies && movies.length > 0) {
    filmList = new Array(Math.floor(movies.length / 8) + 1);
    filmList.forEach((filmSlider) => {
      filmSlider = [];
    });

    for (let i = 0; i < filmList.length - 1; i++) {
      filmList[i] = movies.slice(i * 8, (i + 1) * 8);
    }
    let beg = (filmList.length - 1) * 8;
    filmList[filmList.length - 1] = movies.slice(beg, movies.length);
  }
  ...
  const handleOpen = (id, idx) => {
    setOpen(true);
    let index = movies.findIndex((film) => film.maPhim === id);
    if (index !== -1) {
      setStateFilm({
        id: id,
        index: idx,
        trailer: movies[index].trailer,
      });
    }
  };
